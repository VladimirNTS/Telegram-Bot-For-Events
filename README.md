...
#### Роли:
- Клиент - имеет доступ только к боту. Может записаться на мероприятия, отправлять заявки на партнера и выставку
- Менеджеры - Имееют ограниченный доступ к админке, Админ настрраивает доступ к отдельным вкладкам для каждого менеджера. Менеджер не может добавлять других менеджеров
- Админ - Один на весь проект. Имеет полный доступ ко всем функциям админ панели и может добавлять/удалять других менеджеров.
...
Также через настройки в отдельной вкладке происходит управляние менеджерами (Доступно только для админа).
Можнно добавлять, удалять, редактировать менеджеров. Для создания менеджера нужно заполнить следующие поля:
- Логин
- Пароль (генерируеться автоматически, но можно поменять на свой)
- Разделы к которым менеджер будет иметь доступ (Мероприятия, связь с клиентом, отзывы и тд.)

#### 2.15 Связь с клиентами
Содержит таблицу всех обращений в поддержку с полями:
- Вопрос
- дата
- раздел клиента (партнёр, выставка, мероприятия)
...
При нажатие, открывается чат в админке, где можно ответить человеку. Следующие сообщения от клиента будут приходить в этот же чат.
...
Если человек стоит в очереди, то когда места появяться, бот напишет пользователю о том что для него появилась место и отправит кнопку оплатить, если мероприятие бесплатное, то кнопка "подтвердить". Если в течение 24 часов не подтвердил, бот удаляет сообщение с оплатой, чтобы пользователь не оплатил, ждет 10 минут, на случай если пользователь все таки успеет оплатить в послежнюю минуту, если оплата так и не прошла, бот отпраляет сообщение о том что пользователь не оплатил запись в течение 24 часов и он возвращает его в очередь. После этого очередь идет к следующему
